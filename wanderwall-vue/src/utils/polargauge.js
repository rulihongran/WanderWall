...
export function drawGauge(formObject) {
    ...
    // 发送POST请求，获取旅行记录数据
    request.post('/user/getMapPolar', {province:formObject.province,time:formObject.time,location:formObject.location})
    .then(response => {
        city_data = { //显示的数据
            "name": 'Explored Cities',
            "num": response.city_data
        };
        people_data = { //显示的数据
            "name": 'Rank',
            "num": response.people_data
        };
