 export function initKG(data, config, container) {
    ...
    if (!config.nodeColor || config.nodeColor === "")
        var defaultNodeColor = [
            //粉红
            { fill: "rgb(232,190,232)", stroke: "rgb(162, 84, 162)", text: "rgb(162, 84, 162)" },
            //灰色
            { fill: "rgb(112, 202, 225)", stroke: "#23b3d7", text: "rgb(11,79,129)" },
            { fill: "rgb(201,201,200)", stroke: "rgb(158,164,164)", text: "#333" },
            { fill: "#D9C8AE", stroke: "#c0a378", text: "rgb(152,97,13)" },
            { fill: "rgb(178, 229, 183)", stroke: "rgb(98, 182, 105)", text: "rgb(6,84,42)" },
            //红
    ...
    Object.keys(data.links).forEach(function (key) {
        var type = data.links[key].source == null ? (data.links[key].type = "default") : data.links[key].source.type
    ...
    var width = config.width? config.width:1560,
        height = config.height? config.height:900;
    ...
    var edges_line = svg.append("g").selectAll(".edgepath")
        ...
        .style("stroke", function (link) {
            //config:边框色
            return colorLinkDict[link.source.type].color;
        })//设置线条颜色
    ...
    var circle = svg.append("g")
        ...
        .on("click", function (node) {
            ...
            circle.style('stroke-width', 1.5);//所有的圆圈边框
            //焦点取反
        ...
        .style('fill', function (node) {
            ...
            //如果小于8个字符，不换行
                ...
                d3.select(this).append('tspan')
                    ...
                    .attr('y', 2)
                    .text(function () { return d.name; });
            } else if (d.name.length >= 8) {//大于8个字符时，将7个字后的内容显示为。。。
                var top = d.name.substring(0, 5);
                var bot = d.name.substring(5, 7) + "...";
